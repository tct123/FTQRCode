...
class CryptoHandlerMaster:
    ...
    def validate_data(self, data: str):
        ...
        try:
            ...
            pass
        return False, {"name": "Invalid Data"}
...
class CameraMaster:
    def __init__(
        self,
        page: ft.Page,
        camera_img_control: ft.Image | ft.Container,
        is_a_qr_reader: bool = False,
        qr_reader_callback: callable = None,
    ):
        self.page = page
        ...
        while not self.stop_event.is_set():
            ...
            if self.ret:
                if self.is_a_qr_reader:
                    ...
                    self.drawrect(
                        self.frame, top_left, bottom_right, color, thickness, style
                    )

                    # Scan QR codes within the rectangle
                    self.frame, valid_objects = self.scan_qr_codes_within_rect(
                        self.frame, top_left, bottom_right
                    )
                    if valid_objects and self.qr_reader_callback:
                        for obj in valid_objects:
                            if self.qr_reader_callback and obj["is_valid"]:
    ...
    def scan_qr_codes_within_rect(self, frame, rect_pt1, rect_pt2):
        ...
        for obj in decoded_objects:
            is_valid, qr_data = self.crypto_handler.validate_data(
            ...
            if (
                rect_pt1[0] <= center_x <= rect_pt2[0]
                and rect_pt1[1] <= center_y <= rect_pt2[1]
            ):
...
def main(page: ft.Page):
    ...
    page.add(img)
    page.update()
    camera_master = CameraMaster(
        page,
        img,
        is_a_qr_reader=True,
        qr_reader_callback=lambda data: print(f"QR Code Data: {data}"),
    )
